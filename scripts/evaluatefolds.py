...
def eval_fold(*, results, qrels_path, fold_path, budget, decay_factor,
              nshards, nbuckets, shard_costs):
    ...
    with open(os.path.join(fold_path, 'queries'), 'r') as query_file:
        ...
        queries = np.array([int(query) for query in file_content.split()])
    ...
    p10, map_measure = trec_eval(fold_path=fold_path,
